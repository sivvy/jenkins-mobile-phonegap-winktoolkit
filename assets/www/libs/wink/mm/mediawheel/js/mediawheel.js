...
/**
 * Implements a media wheel. The media wheel is a progress bar, displayed as a wheel, used for media files and based on the Canvas tag.
 * 
 * @methods:
 * 	--> getDomNode: Returns the DOM node of the component
 * 	--> setDuration: Set the duration
 * 	--> setTime: Set the time
 * 	--> setBufferedTime: Set the buffered time
 * 
 * @properties:
 * 	data =
 * 	{
 ...
 * 		thickness: the thickness of the wheel
 * 		startAngle: the start angle of the wheel
 * 		endAngle: the end angle of the wheel
 * 		readGradientColors: the gradient colors of the read part of the media (eg. [{ pos: 0, color: color1 },{ pos: 0.75, color: color2 },{ pos: 1, color: color3 }])
 * 		bgGradientColors: the gradient colors of the background part of the media (eg. [{ pos: 0, color: color1 },{ pos: 0.75, color: color2 },{ pos: 1, color: color3 }])
 * 		unbufferedGradientColors: the gradient colors of the unbuffered part of the media (eg. [{ pos: 0, color: color1 },{ pos: 0.75, color: color2 },{ pos: 1, color: color3 }])
 * 		cursorRectColor: the color of the cursor's rectangle
 * 		cursorTriangleColor: the color of the cursor's triangle
 * 		canChangeTime: indicates whether the user can change the time by moving or clicking on the wheel (default is true)
 * 	}
 * 
 * @attributes:
 ...
 *
 * @events
 * 	--> /mediawheel/events/timeupdate: a time update
 *
 * @dependencies:
 *  --> wink.math.geometric
...
define(['../../../_amd/core', '../../../math/_geometric/js/geometric', '../../../ux/movementtracker/js/movementtracker'], function(wink)
{
	...
	{
		var pos = _getAbsolutePosition(node),
			offsetX = absX - pos.x,
			offsetY = absY - pos.y,
			xTrigo = (offsetX - cx),
			yTrigo = -(offsetY - cy),
