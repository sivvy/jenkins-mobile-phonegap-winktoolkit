...
(function() {
	...
	var Configuration = {
        ...
        Config = function(){
        	this.config = {};
        	this.configArray = [];
        	jQuery.extend( this.config, Configuration );
        	this.configArray = this.generateArray();
        };
        Config.prototype.generateArray = function() {
        	var local = jQuery.parseJSON( window.localStorage.getItem( "config" ) ),
        	   config = this.config;
        	config = jQuery.extend( config, local );
            var i, array = [];
            for( i in config ) {
                config[i].id = i;
                array.push( config[i] );
        ...
        Config.prototype.save = function() {
            window.localStorage.setItem( "config", jQuery.toJSON( this.config ) );
            this.configArray = this.generateArray();
        }
        
    c = new Config();
	jenkins.Config = c;
	jenkins.serverChanged = false;
})();